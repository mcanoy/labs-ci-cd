...
module.exports = (robot) ->
  ...
  robot.router.post '/hubot/notify/:room', (req, res) ->
    ...
    message = "#{robot.name} says " 
    message += req.body.message.replace(/\.j proceed/i, "#{robot.name}  jenkins proceed")
    message += message.replace(/\.j abort/i, " #{robot.name}  jenkins abort")
